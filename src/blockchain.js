...
class Blockchain {
  ...
  async _addBlock(block) {
    try {
      ...
      block.previousBlockHash = chain[height - 1]
        ? chain[height - 1].hash
        : null;
      block.height = height;
  ...
  async getStarsByWalletAddress(address) {
    const ownedBlocks = this.chain.filter((b) => b.owner === address);
    if (ownedBlocks.length === 0) {
      throw new Error("Address not found.");
    }
    try {
      const stars = ownedBlocks.map((block) =>
        JSON.parse(hex2ascii(block.body))
      );
      return stars;
    } catch (e) {
      console.log(e);
      throw new Error("Failed to return stars.");
    }
  }
   ...
   */
  async validateChain() {
    const promises = this.chain.map((block, index) => {
      return new Promise(async (resolve, reject) => {
        const isValid = await block.validate();
        if (!isValid) {
          reject(new Error(`Invalid block #${block.height}: ${block.hash}`));
        }
        if (block.height > 0) {
          const previousBlock = this.chain[index - 1];
          if (block.previousBlockHash !== previousBlock.hash) {
            reject(
              new Error(`Invalid block Chain #${block.height}: ${block.hash}`)
            );
          }
        }
        resolve();
